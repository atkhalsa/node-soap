...
- `options` (*Object*):
  ...
  - `flattenArrayElements` (*boolean*): flattens array elements. If `true`, elements of JSON array is create on the one level up. ex. `{list: [{elem: 1}, {elem: 2}]}` are marshalled into xml as `<elem>1</elem><elem>2</elem>`. (**Default**: `false`)
